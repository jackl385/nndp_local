...
def m(key:jax.random.PRNGKey, state:Array, action:Array) -> Array:
    ...
    return jnp.column_stack([z_next, k_next])
...
def F(key:jax.random.PRNGKey, N:int) -> Array:
    ...
    z = jax.random.uniform(subkey[0], shape = (N,), minval=z_bound[0], maxval=z_bound[1])
    k = jax.random.uniform(subkey[1], shape = (N,), minval=k_bound[0], maxval=k_bound[1])
    return jnp.column_stack([z, k])
...
def nn_to_action(state: jnp.ndarray, 
    ...
    z, k = state[:, 0], state[:, 1]
    state_norm = jnp.column_stack([z, k])
